...
SDNU 工具箱是一款面向**山东师范大学**学生的本地校园效率桌面软件，基于 [QLU 工具箱](https://github.com/C1ouDreamW/qlu-toolbox) 改造而来。使用 Vue 3、TypeScript 与 Electron 提供桌面界面，Python 与 Playwright 负责可靠的本地自动化。当前内置「分项成绩导出」和「绩点计算器」两个教务工具。
...
本项目基于 [C1ouDreamW/qlu-toolbox](https://github.com/C1ouDreamW/qlu-toolbox) 改造，感谢原作者的开源贡献。
...
2. 若无法访问 GitHub，可从蓝奏云下载（待补充）。
3. **校外访问**：启动后会自动打开 WebVPN 门户页面，依次完成：
   - WebVPN 统一身份认证登录
   - 在 VPN 门户中找到并进入「教务系统」
   - 在教务登录页输入学号密码
4. **校内访问**：直接连接校园网即可使用。
...
- 使用安装程序安装时，关闭正在运行的程序再运行新版安装程序即可覆盖升级。
- 上述更新方式都会保留设置、任务记录、日志和浏览器登录状态。
...
- **校外访问需要 WebVPN**，登录等待时间 15 分钟。
- SDNU 教务系统无分项成绩导出接口，成绩通过页面抓取获取。
- 当前提供 Windows x64 安装包和免安装 ZIP。
